...
const CommentItem = ({ comment, activeUser }) => {
    ...
    const getCommentLikeStatus = async () => {
      ...
      try {
        const { data } = await axios.post(
          `${process.env.REACT_APP_API_URL}/api/comment/${comment_id}/getCommentLikeStatus`,
          { activeUser },
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const editDate = (createdAt) => {
    const d = new Date(createdAt);
    return d.toLocaleString("eng", { month: "short" }) + " " + d.getDate();
  };
  ...
  const handleCommentLike = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/comment/${comment_id}/like`,
        { activeUser },
        {
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="comment-item">
      <div className="comment-top-block">
        <section>
          <img
            src={`${process.env.REACT_APP_API_URL}/userPhotos/${comment.author.photo}`}
